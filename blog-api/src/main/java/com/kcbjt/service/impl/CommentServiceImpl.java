...
import com.kcbjt.mapper.CommentMapper;
import com.kcbjt.model.vo.PageComment;
import com.kcbjt.service.CommentService;
...
public class CommentServiceImpl implements CommentService {
	...
	@Override
	public List<com.kcbjt.entity.Comment> getListByPageAndParentCommentId(Integer page, Long blogId, Long parentCommentId) {
		List<com.kcbjt.entity.Comment> comments = commentMapper.getListByPageAndParentCommentId(page, blogId, parentCommentId);
		for (com.kcbjt.entity.Comment c : comments) {
			//递归查询子评论及其子评论
			List<com.kcbjt.entity.Comment> replyComments = getListByPageAndParentCommentId(page, blogId, c.getId());
	...
	@Override
	public com.kcbjt.entity.Comment getCommentById(Long id) {
		com.kcbjt.entity.Comment comment = commentMapper.getCommentById(id);
		if (comment == null) {
	...
	public void updateCommentPublishedById(Long commentId, Boolean published) {
		...
		if (!published) {
			List<com.kcbjt.entity.Comment> comments = getAllReplyComments(commentId);
			for (com.kcbjt.entity.Comment c : comments) {
				hideComment(c);
			}
		}
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public void updateComment(com.kcbjt.entity.Comment comment) {
	 ...
	 */
	private void delete(com.kcbjt.entity.Comment comment) {
		for (com.kcbjt.entity.Comment c : comment.getReplyComments()) {
			delete(c);
		}
	 ...
	 */
	private void hideComment(com.kcbjt.entity.Comment comment) {
		for (com.kcbjt.entity.Comment c : comment.getReplyComments()) {
			hideComment(c);
		}
	 ...
	 * @return
	 */
	private List<com.kcbjt.entity.Comment> getAllReplyComments(Long parentCommentId) {
		List<com.kcbjt.entity.Comment> comments = commentMapper.getListByParentCommentId(parentCommentId);
		for (com.kcbjt.entity.Comment c : comments) {
			List<com.kcbjt.entity.Comment> replyComments = getAllReplyComments(c.getId());
			c.setReplyComments(replyComments);
		}
		return comments;
	}
